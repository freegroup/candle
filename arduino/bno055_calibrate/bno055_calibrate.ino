...
void setup()
{
  ...
  if (!bno.begin(BNO055::OPERATION_MODE_COMPASS)) //if you want to calibrate using another mode, set it here. OPERATION_MODE_COMPASS for a precise tilt compensated compass (Section 3.3.2 / 3.3.3)
  {
...
void loop()
{
  ...
  bool bMAG = bno.getCalibState(BNO055::CALIB_STAT_MAG);
  Serial.print("MAG Calibration: ");
  Serial.println(bMAG);
  bool bGYR = bno.getCalibState(BNO055::CALIB_STAT_GYR);
  Serial.print("GYR Calibration: ");
  Serial.println(bGYR);
  bool bACC = bno.getCalibState(BNO055::CALIB_STAT_ACC);
  Serial.print("ACC Calibration: ");
  Serial.println(bACC);
  bool b = bno.getCalibState(BNO055::CALIB_STAT_SYS);
